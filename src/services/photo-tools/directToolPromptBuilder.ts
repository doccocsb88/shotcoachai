...
export function buildDirectToolImagePrompt(tool: PhotoAiTool, instruction?: string): string {
  switch (tool.id) {
    case 'enhance_photo':
      return directToolPromptBuilder.enhancePhoto(instruction);
    case 'better_composition':
      return directToolPromptBuilder.compositionOnly(instruction);
    case 'light_color':
      return directToolPromptBuilder.lightColor(instruction);
    case 'upscale':
      return directToolPromptBuilder.upscale(instruction);
    case 'background_boost':
      return directToolPromptBuilder.backgroundBoost(instruction);
    case 'expand_frame':
      return directToolPromptBuilder.expandFrame(instruction);
    case 'replace_background':
      return directToolPromptBuilder.replaceBackground(instruction);
    case 'remove_object':
      return directToolPromptBuilder.removeObject(instruction);
    case 'smooth_skin':
      return directToolPromptBuilder.skinOnly(instruction);
    default:
      return directToolPromptBuilder.genericDirectTool(tool, instruction);
  }
}

const directToolPromptBuilder = {
