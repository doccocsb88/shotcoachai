...
export class DirectCoachService {
  ...
  static getPromptForMode(mode: CoachMode, coachPreferences?: CoachPreferences): string {
    ...
    switch (mode) {
      ...
      case 'frame':
        return contextPrefix + basePrompt + 'Adjust the zoom level, camera distance, and crop to show the ideal framing for this shot (e.g. Medium Shot, Full Body, or Close-up depending on the context).';
      case 'angle':
      ...
      case 'pose':
        return contextPrefix + basePrompt + 'Improve the subject\'s body posture and pose to be more aesthetically pleasing, natural, and confident, without changing their face or clothes. Make the pose look professional.';
      case 'comprehensive':
        return contextPrefix + basePrompt + 'Improve the composition, framing, and pose all at once for a perfect, professional aesthetic photo.';
      default:
