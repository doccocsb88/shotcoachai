...
import { AnalysisResult, getFlowType, getStoredGenerationUri } from '../../models/analysis';
...
interface Props {
  children: ReactNode;
  launchPaywallSource?: LaunchPaywallSource | null;
}

export function AppNavigationProvider({ children, launchPaywallSource = null }: Props) {
  ...
  const handlePhotoSelected = useCallback(() => {
    if (cameraIntent.type === 'coach') {
      ...
      const capturedPhoto = analysisState.currentPhoto;
      const existingCoachResult = analysisState.currentResult;
      const coachReferenceImageUri =
        coachReferenceUri
        ?? (existingCoachResult ? getStoredGenerationUri(existingCoachResult, 0) : undefined)
        ?? existingCoachResult?.generatedImageUri;

      if (!capturedPhoto || !coachReferenceImageUri) {
        openHome();
        return;
      }

      const coachCaptureResult: AnalysisResult = {
        ...
        originalImageUri: capturedPhoto.uri,
        originalImageMimeType: capturedPhoto.mimeType,
        generatedImageUri: coachReferenceImageUri,
        suggestionGenerations: [{ suggestionIndex: 0, generatedImageUri: coachReferenceImageUri }],
        selectedSuggestionIndex: 0
      };
      setCurrentResult(coachCaptureResult);
      navigate('GeneratedResult', {
        suggestionIndex: 0,
        openedFromHistory: false,
        canReturnToAnalysis: false
  ...
  useEffect(() => {
    if (!launchPaywallSource) {
      return;
    }

    if (launchPaywallSource === 'app_open' && UserManager.getState().isPremium) {
      return;
    }

    const timeoutId = setTimeout(() => {
      openPaywall('DirectStore', launchPaywallSource);
    }, 350);

    return () => {
      clearTimeout(timeoutId);
    };
  }, [launchPaywallSource, openPaywall]);
